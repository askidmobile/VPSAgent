...
**Критерии приёмки:**
- [x] После `vpsagent daemon` и закрытия SSH процесс `vpsagent` остаётся в `ps`
      и продолжает слушать сокет.
- [x] PID в lock-файле совпадает с реальным pid процесса-демона.
- [x] Логи пишутся в файл внутри data_dir; путь сообщается при запуске.
- [x] Повторный `vpsagent daemon` отказывается стартовать («демон уже запущен,
...
**Критерии приёмки:**
- [x] `--foreground` отключает daemonize независимо от типа stdin.
- [x] Ctrl+C → graceful shutdown (тот же путь, что сейчас по SIGTERM).
- [x] Логи идут в stderr, не в файл.
...
**Критерии приёмки:**
- [x] `status` показывает version, pid, uptime, sessions, agents.
- [x] `status` при незапущенном демоне — корректное сообщение, exit code ≠ 0.
- [x] `stop` инициирует graceful shutdown: агенты отменяются, сокет/lock чистятся.
- [x] `stop` при незапущенном демоне — понятное сообщение, exit code ≠ 0.
...
- [x] `vpsagent daemon` на Linux переживает закрытие SSH (процесс жив в `ps`,
      сокет отвечает). *Проверено: SIGHUP игнорируется, status отвечает (Фаза 3/5).*
- [x] `vpsagent daemon --foreground` работает в терминале с логами в stderr и
      Ctrl+C-завершением. *Проверено: Фаза 3, SIGTERM → graceful shutdown.*
- [x] `vpsagent daemon stop` корректно останавливает демона (сокет/lock чистятся,
      агенты отменяются). *Проверено: Фаза 3/5, «демон остановлен», сокет очищен.*
- [x] `vpsagent daemon status` показывает version/pid/uptime/sessions/agents и
      корректно сообщает о незапущенном демоне. *Проверено: text + json (Фаза 3).*
- [x] Повторный запуск `vpsagent daemon` при живом демоне отказывается стартовать
      с осмысленным сообщением. *Реализовано через DaemonLock (FR-005).*
- [x] Сборка под `x86_64-unknown-linux-musl` проходит, бинарь daemonize работает
      в musl-окружении (проверено smoke-тестом: старт, закрытие «SSH»-имитации,
      проверка живости). *Фаза 5: exit 0, SIGHUP-игнор, stop — всё работает в musl.*
- [x] Сборка на Windows/macOS не ломается (foreground-only, компилируется).
      *macOS: cargo check --workspace — OK (Фаза 5). Windows: пути компилируются,
      runtime не проверен (нет toolchain).*
- [x] `ensure_daemon` (авто-подъём из клиента) использует общий код daemonize и
      поведение не регрессирует. *Фаза 4: spawn_detached_daemon, smoke vpsagent run.*